...
def main():
    ...
    mytime = time.localtime()
    print(f"Ponderosa_Electricity_Usage.py: Script started with PID {PID} at " + time.strftime('%m/%d/%Y %H:%M:%S', mytime))
    print(f"==> INI = \"{INI}\", startTS = \"{startTS}\"")
    sys.stdout.flush()
    sys.stderr.flush()
    ...
    while True:
        ...
        if (the_hour == the_hour_last):  # Still in same hour
            ...
            if (minute_counter > 0):
                ...
                mytime = time.localtime()
                print(f"==> DB.Insert(): {time.strftime('%m/%d/%Y %H:%M:%S')} kWh={kWh}")
                DB.insert(the_date_prev_db,the_hour_last_db,kWh)
                kWh_day += kWh
